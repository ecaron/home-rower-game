...
var chartColors = {
  ...
  var adjustedToggles = JSON.parse(localStorage.getItem('toggles'))
  var visibleGraphs = 0
  var graphs = {}
  ...
  function resizeGraphs() {
    if (mobileCheck() === false) {
      var divider = Math.ceil(Math.sqrt(visibleGraphs))
      var gridWidth = Math.floor((window.innerWidth - 50) / divider)
      var gridRows = 1
      while (gridRows * divider < visibleGraphs) gridRows++
      var gridHeight = Math.floor((window.innerHeight - toggleDiv.offsetHeight - 30) / gridRows)
