...
exports.init = function (server, sessionParser) {
  ...
  wss.on('connection', function (ws, req) {
    ...
    const rowerPosition = function (target, metric, value) {
      ...
      if (target === 'rower') {
        if (metric === 'speed' && req.session.recording.maxSpeed < value) {
    ...
    ws.on('message', function (message) {
      ...
      if (data.status === 'end') {
        rower.saveRecording(userId, req.session.recording, function (personalBest) {
          debug(`Got personal best of ${JSON.stringify(personalBest)}`)
          if (req.session.recording.maxSpeed) {
            ...
            debug(`Rowers results: ${JSON.stringify(results.rower)}`)
          }
          debug(`Comparing your distance of ${results.rower.distance} to theirs of ${results.competitor.distance}`)
          if (parseInt(results.rower.distance, 10) >= parseInt(results.competitor.distance, 10)) {
            results.winner = 'YOU'
          } else {
            results.winner = 'THEM'
          }
          results.personalBest = personalBest
          db.rowers.update({ _id: req.session.userId }, { $set: { recentRace: results } }, function(err){
            if (err) debug(err)
    ...
    if (process.env.FAKE_ROWER) {
      ...
      S4.rower.event.on('update', function (data) {
        if (data.name === 'ms_distance') rowerPosition('rower', 'distance', data.value.toFixed(2))
        if (data.name === 'ms_average') rowerPosition('rower', 'speed', data.value.toFixed(2))
      })
    }
